...
import br.com.simulator.credit.creditas.commondomain.valueobjects.*
import br.com.simulator.credit.creditas.simulationdomain.model.*
import br.com.simulator.credit.creditas.simulationdomain.model.ports.SimulationPersistencePort
import br.com.simulator.credit.creditas.simulationdomain.model.valueobjects.*
...
internal class SimulateLoanCommandHandlerTest {
  ...
  @BeforeEach
  fun setup() {
    handler = SimulateLoanCommandHandler(publisher, loanAmountFactory, persistencePort)
  }

  @Test
  fun `should execute simulation and return response`() = runBlocking {
    ...
    val aggregate = SimulateLoanAggregate(
      id = LoanSimulationId(),
    ...
    val policy = mockk<InterestRatePolicy> {
      every { annualInterestRate(any()) } returns BigDecimal("0.03")
    }

    val command = SimulateLoanCommand(
      amount = amount,
